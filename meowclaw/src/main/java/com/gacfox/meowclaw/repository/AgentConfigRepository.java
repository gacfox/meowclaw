...
public class AgentConfigRepository {
    ...
    private AgentConfig insert(AgentConfig agent) {
        String sql = "INSERT INTO agent_configs (name, avatar, system_prompt, enabled_tools, default_llm_id, workspace_folder, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    private AgentConfig update(AgentConfig agent) {
        String sql = "UPDATE agent_configs SET name = ?, avatar = ?, system_prompt = ?, enabled_tools = ?, default_llm_id = ?, workspace_folder = ?, created_at = ?, updated_at = ? WHERE id = ?";
        jdbcTemplate.update(sql,
                agent.getName(),
                agent.getAvatar(),
                agent.getSystemPrompt(),
