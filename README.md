...
- `memory_write`：写入记忆工具。智能体调用该工具后，MeowClaw会先自动召回与新内容最相似的已有记忆，然后调用辅助LLM进行决策：重复内容跳过写入；与已有记忆矛盾或使其失效的内容会删除对应旧记忆；描述同一事实但更准确的内容会更新已有记忆；其余情况正常插入。插入的记忆会被抽取为记忆和实体关系，并使用Lucene9存储倒排索引和向量（如果配置了嵌入模型），记忆的存储模型是轻量级的图结构，记忆数据通过实体互相关联。
